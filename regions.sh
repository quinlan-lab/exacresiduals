...
while getopts ":v:x:d:r:q:segcnwf:" opt; do
    case $opt in
        ...
        d)
            ...
            ;;
        r)
            echo "chromosomes to include (all but Y default)" >&2
            r="-p"
            chrom+=("$OPTARG")
            echo "-chrom input: '${chrom[@]}'" >&2
            ;;
        q)
            echo "chromosomes to exclude (only Y default)" >&2
            q="-x"
            nochrom+=("$OPTARG")
...
python resid-plot.py $ns $syn $cpg $var $r "${chrom[@]}" $q "${nochrom[@]}" -f results/$date/exac-regions$n$w.txt > results/$date/resids$c$s$n$w.txt
